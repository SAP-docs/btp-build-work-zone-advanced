...
-   The `X-Frame-Options` header should be either:
    ...
    -   Make sure you're using Identity Authentication as the authentication mechanism as a proxy mode - in this case the `X-Frame-options` dont exist.
...
1.  Open the Identity Authentication admin environment.
    ...
    For example, `https://dwp.accounts.ondemand.com/admin`
...
As the XSUAA service and SAP Build Work Zone, advanced edition are deployed on the same environment, no additional configuration is needed.
...
You can enable CSRF protection by the approuter \(previously it was assumed that the backends behind the destinations handled this\).

To enable this protection, set the `CEP.EnableCsrfProtection` destination property to true for all dynamic destinations where their respective backend doesn’t handle CSRF protection.
...
Local approuters - configure cookies in the approuter as an environment variable

</td>
<td valign="top">

If you're integrating a content provider that uses a local approuter that is not managed by SAP, make sure to configure the required cookies as an approuter environment variable. For more information, see [Environment Variables](https://help.sap.com/docs/BTP/65de2977205c403bbc107264b8eccf4b/ba527058dc4d423a9e0a69ecc67f4593.html).
